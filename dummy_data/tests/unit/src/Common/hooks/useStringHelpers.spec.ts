...
describe('useStringHelpers', () => {
  ...
  test.each([
    {
      sentence:
        'columbo is a serie with peter falk acting as columbo character',
      separator: 'columbo',
      result: [
        'columbo',
  ...
  test.each([
    ...
    'StringFormat when text format is "%s" and expected "%s" with args %p',
    (text: string, expected: string, ...args: string[]) => {
  ...
  test.each([
    ...
    'validStringAlphanumericFrench when text is "%s" should be "%p"',
  ...
  test.each([
    ...
    'convertStringIntoBoolean when text is "%s" should be "%p"',
    (text: string | undefined | null, expected: boolean) => {
