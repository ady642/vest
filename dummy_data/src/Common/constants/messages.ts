const messages = {
    ...
    delete: {
      modal: {
        ...
        description: (folderName: string) =>
          `Si vous cliquez sur "Continuer", tous les fichiers et sous dossiers à partir du dossier "${folderName}" seront supprimés et envoyés à la corbeille.`
      },
      error: "Vous n'avez pas les droits nécessaires pour supprimer ce dossier"
    },
    create: {
      error:
        "Ce dossier est en lecture seule. Vous ne bénéficiez pas des droits d'accès à son édition.",
      validationNameWarning:
        'Veuillez nommer votre dossier en utilisant des lettres ou des chiffres.'
    }
  },
  documents: {
    delete: {
      modal: {
        ...
        description: (syncStatus: boolean) =>
          syncStatus
  ...
  trash: {
    card: {
      title: 'Corbeille',
      description:
        "Ici, retrouvez l'ensemble des documents supprimés. Vous pourrez les restaurer ou les archiver."
    }
  }
}
